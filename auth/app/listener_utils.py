...
def location_request(first_msg_obj):
    expected = ["location"]
    real = sorted(list(first_msg_obj.keys()))
    if expected != real:
        print("Invalid message structure")
    ...
    # check structure of content
    expected = ["safe", "signature", "ts", "username"]
    real = sorted(list(content.keys()))
    if expected != real:
        print("Invalid content structure")
        return None, '{"error": "Invalid content structure"}'
    ...
    safe = str(content["safe"])
    if safe not in ["OK", "NO"]:
        print("invalid 'safe' value")
        return None, '''{"error": "invalid 'safe' value"}'''
    ...
    # Verify signature
    to_sign = (username + str(ts) + safe).encode()
    pub_key = certificate.public_key()
    try:
        pub_key.verify(signature,
                       to_sign,
    ...
    return (username, ts_int, secret_key, safe), None
